/*
 https://www.codechef.com/problems/TASTR
 https://discuss.codechef.com/questions/7697/tastr-editorial
 */
...
int main(void)
{
    ...
    n = strlen(a.c_str());
    m = strlen(b.c_str());
    /*to find the unique substrings of a and b
     a + b - 2(a INTERSECTION b)
     by Inclusion Exclusion Principle
     2 (a UNION b) - a - b
     for a UNION b, we make a string a$b
     but this new string will have substrings containing $ that also has to be excluded from our calculations of unique substrings
     there are (n+1)*(m+1) such substrings which need to be substracted
    */
    cout<<2*(makeSuffixArray(s,n+m+1) - (n+1)*(m+1) ) - makeSuffixArray(a,n) - makeSuffixArray(b,m)<<endl;
    return 0;
}